...
const BANDSINTOWN_URL = 'https://www.bandsintown.com/a/15493253-dalai-puma';
...
const Shows = () => {
  ...
  const getGigs = () => {
    const url =
      'https://rest.bandsintown.com/artists/dalai%20puma/events?app_id=mockingbird&date=all';
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        setGigs(data);
      ...
      <div style={{ width: '100%' }}>
        ...
        >
          <button
            type="button"
            active={upcoming}
            style={{ backgroundColor: upcoming ? 'magenta' : 'black' }}
        ...
        {filteredGigs.map((concert) => {
          ...
          return (
            <div className="concert" key={concert.datetime}>
              <p>
                <span>
                  {`${dd} ${mm} ${yyyy}: `}
                  {!upcoming ? (
                    ...
                    <a
                      ...
                      href={concert.url || BANDSINTOWN_URL}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {concert.venue.name}
                  ...
                  in {concert.venue.city}, {concert.venue.country}
                </span>
              </p>
            </div>
          );
